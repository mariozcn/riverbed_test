...
def test_user_events_returns_only_that_users_events(client, user):
    other = client.post(
    ...
    client.post(
        "/events",
        json={"user_id": other["id"], "event_type": "click", "metadata": {}},
    )
...
def test_user_events_missing_user_returns_404(client):
    response = client.get("/users/9999/events")
    assert response.status_code == 404


def test_user_events_filters_by_since(client, user):
    ...
    all_events = client.get(f"/users/{user['id']}/events").json()
    # use the timestamp of the first event as the cutoff: events created
    # strictly after it should remain (the first one is excluded)
    cutoff = all_events[0]["created_at"]

    response = client.get(f"/users/{user['id']}/events?since={cutoff}")
    assert response.status_code == 200
    returned_ids = {e["id"] for e in response.json()}
    assert all_events[0]["id"] not in returned_ids
    assert len(response.json()) == 2


def test_user_events_excludes_soft_deleted(client, user):
    created_ids = []
    for _ in range(2):
        response = client.post(
            "/events",
    ...
    client.delete(f"/events/{created_ids[0]}")
